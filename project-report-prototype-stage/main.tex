...
See \texttt{docs/uml/use-case-diagram.md} in the repository for the
PlantUML source and full written use-case scenarios. Render the
PlantUML block at \url{https://www.plantuml.com/plantuml/} (or any
local PlantUML renderer), export as PNG/SVG, and place it here via
...
See \texttt{docs/uml/class-diagram.md} in the repository for the
PlantUML source and the accompanying design-decision notes (the
nullable \texttt{shg\_id} dual-path pivot, append-only
\texttt{CreditScore} history, and the \texttt{SHGLenderLink}
many-to-many approve/reject table).

% \includegraphics[width=\textwidth]{class-diagram.png}

\section{Activity Diagram}
\label{sec:activity-diagram}
See \texttt{docs/uml/activity-diagram.md} in the repository for the
PlantUML source (with Borrower / Lender / System swimlanes) covering
the full request $\to$ approval $\to$ repayment $\to$ rescoring flow,
and a prose walkthrough mapping each step to the real backend
